...
const getBooks = async (req, res) => {
  try {
    ...
    const sellerIds = [...new Set(books.map(book => book.seller_id).filter(Boolean))];
    ...
    const booksWithImages = books.map(book => {
      ...
      return {
        ...book,
        coverimageurl: book.image_url || null,
        images: book.image_url ? [book.image_url] : [],
        vendedor: seller ? 
          `${seller.first_name || ''} ${seller.last_name || ''}`.trim() || 
          seller.username || 
          seller.email : 'No especificado',
        categoria: category ? (category.category_name || category.name || 'No especificada') : 'No especificada'
      };
    });

    console.log('Enviando respuesta con', booksWithImages.length, 'libros');
